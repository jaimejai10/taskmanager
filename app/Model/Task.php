...
function insert_task($conn, $data){
	$sql = "INSERT INTO tasks (report_id, title, description, company_name, assigned_to, due_date) VALUES(?,?,?,?,?,?)";
...
function update_task($conn, $data){
	$sql = "UPDATE tasks SET title=?, description=?, assigned_to=?, due_date=? WHERE task_id=?";
	$stmt = $conn->prepare($sql);
	$stmt->execute($data);
}
...
function count_pending_tasks($conn){
	$sql = "SELECT task_id FROM tasks WHERE status = 'pending'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function count_in_progress_tasks($conn){
	$sql = "SELECT task_id FROM tasks WHERE status = 'in_progress'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function count_my_tasks($conn, $id){
	$sql = "SELECT task_id FROM tasks WHERE assigned_to=?";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_my_tasks_overdue($conn, $id){
	$sql = "SELECT task_id FROM tasks WHERE due_date < CURDATE() AND status != 'completed' AND assigned_to=? AND due_date != '0000-00-00'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_my_tasks_NoDeadline($conn, $id){
	$sql = "SELECT task_id FROM tasks WHERE assigned_to=? AND status != 'completed' AND due_date IS NULL OR due_date = '0000-00-00'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_my_pending_tasks($conn, $id){
	$sql = "SELECT task_id FROM tasks WHERE status = 'pending' AND assigned_to=?";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_my_in_progress_tasks($conn, $id){
	$sql = "SELECT task_id FROM tasks WHERE status = 'in_progress' AND assigned_to=?";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_my_completed_tasks($conn, $id){
	$sql = "SELECT task_id FROM tasks WHERE status = 'completed' AND assigned_to=?";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
